...
func main() {
	...
	e.GET("/api/users/:id", func(c echo.Context) error {
		...
		rows, err := db.Query("SELECT r.*, s.rank AS sheet_rank, s.num AS sheet_num FROM reservations r INNER JOIN sheets s ON s.id = r.sheet_id WHERE r.user_id = ? LIMIT 5", user.ID)
		if err != nil {
			return err
		}
		defer rows.Close()
		...
		rows, err = db.Query("SELECT event_id FROM reservations WHERE user_id = ? GROUP BY event_id DESC LIMIT 5", user.ID)
		if err != nil {
			return err
		}
		defer rows.Close()
